const text_box = '<div class="card-panel right" style="width: 51%; position: relative">' +
    ...
    '<img src={is_read} style="width:20px; margin-left: auto; display: block;">' +
    '</div>' +
    '</div>';
...
function send(sender, receiver, message) {
    $.post('/api/messages', '{"sender": "' + sender + '", "receiver": "' + receiver + '","message": "' + message + '" }', function (data) {
        ...
        box = box.replace('{is_read}', data.is_read ? '/static/assets/img/double-tick-read.png' : '/static/assets/img/double-tick-read.png' )
        $('#board').append(box);
        scrolltoend();
    })
}
    ...
    $.get('/api/messages/' + sender_id + '/' + receiver_id, function (data) {
        ...
        if (data.length !== 0) {
            for (var i = 0; i < data.length; i++) {
                ...
                box = box.replace('<img src={is_read} style="width:20px; margin-left: auto; display: block;">', '' );
                $('#board').append(box);
                scrolltoend();
            }
        }
